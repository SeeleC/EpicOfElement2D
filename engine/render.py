# -*- coding: utf-8 -*-
"""程序化渲染辅助。
在没有美术贴图的情况下，用 arcade 基本几何图形 绘制「饥荒式」的俯视角色、
怪物、图标与 UI。
- rect / circle / draw_centered_text 为世界/UI 通用绘制工具。
- 图标用简单像素风格：每个图标由若干色块构成。

arcade 3.x：create_* 系列位于 arcade.shape_list。
"""
import arcade
...
def rect(x, y, w, h, color=(200, 200, 200), group=None):
    """绘制填充矩形（屏幕像素坐标）。arcade 以中心点定位，故换算中心。"""
    return shape_list.create_rectangle_filled(x + w / 2, y + h / 2, w, h, color)


def circle(x, y, r, color=(200, 200, 200), segments=None):
    return shape_list.create_ellipse_filled(x, y, r * 2, r * 2, color)


def draw_centered_text(text, cx, cy, font_size=14, color=(255, 255, 255),
                       anchor_x="center", anchor_y="center", bold=False):
    return arcade.Text(text, cx, cy, (*color, 255), font_size=font_size,
...
def icon_sprite(icon_id, x, y, scale=1.0, batch=None, group=None):
    ...
    shape = shape_list.create_rectangle_filled(x + size / 2, y + size / 2, size, size, color)
    if batch is not None:
        batch.add_shape(shape)
    return shape
